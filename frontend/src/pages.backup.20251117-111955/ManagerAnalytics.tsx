                          ...
                          <tbody className="divide-y divide-gray-200">
                            {campaignProgress.campaigns.map((campaign: any) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-4 whitespace-nowrap text-center">
                                  <span className="campaign-badge px-3 py-1.5 bg-cyan-100 text-cyan-800 text-xs font-semibold inline-block rounded-full">
                                    {campaign.duplicate}
                                  </span>
                                </td>
                                <td className="px-4 py-4 whitespace-nowrap text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              {selectedAgent ? (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <StatCard title="Total" value={selectedAgent.summary.total} color="border-cyan-500" />
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {agentReports
                    ...
                    <tr key={agent.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        <span 
                          className={`role-badge px-3 py-1 text-xs font-medium rounded-full ${
                            agent.role === 'Admin' ? 'bg-red-100 text-red-800' :
                            agent.role === 'Manager' ? 'bg-indigo-100 text-indigo-800' :
                            agent.role === 'AccountManager' ? 'bg-pink-100 text-pink-800' :
                            agent.role === 'SeniorAgent' ? 'bg-purple-100 text-purple-800' :
                            agent.role === 'TeamLeader' ? 'bg-cyan-100 text-cyan-800' :
                            agent.role === 'QualityControl' ? 'bg-orange-100 text-orange-800' :
                            agent.role === 'IT' ? 'bg-teal-100 text-teal-800' :
                            'bg-blue-100 text-blue-800'
                          }`}
                        >
                          {agent.role === 'Admin' ? 'Admin' :
                           agent.role === 'Manager' ? 'Manager' :
                           agent.role === 'AccountManager' ? 'ACM' :
                           agent.role === 'SeniorAgent' ? 'Senior' : 
                           agent.role === 'TeamLeader' ? 'TL' :
                           agent.role === 'QualityControl' ? 'QC' :
                           agent.role === 'IT' ? 'IT' : 'Agent'}
                        </span>
                      </td>
                  ...
                  {attendanceReports.map((record) => (
                    <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span 
                          className={`role-badge px-3 py-1 text-xs font-medium rounded-full ${
                            record.role === 'Admin' ? 'bg-red-100 text-red-800' :
                            record.role === 'Manager' ? 'bg-indigo-100 text-indigo-800' :
                            record.role === 'AccountManager' ? 'bg-pink-100 text-pink-800' :
                        ...
                        >
                          {record.role === 'Admin' ? 'Admin' :
                           ...
                           record.role === 'AccountManager' ? 'ACM' :
