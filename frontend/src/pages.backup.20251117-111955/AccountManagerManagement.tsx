                  ...
                  {teamLeaders.length === 0 ? (
                    ...
                    teamLeaders.map(tl => (
                      ...
                      >
                        ...
                        />
                        <span className="ml-2 font-medium text-gray-900">{tl.fullName}</span>
                      </label>
                    ))
                  )}
                </div>
                  ...
                  {qcAgents.length === 0 ? (
                    ...
                    qcAgents.map(qc => (
                      ...
                      >
                        ...
                        />
                        <span className="ml-2 font-medium text-gray-900">{qc.fullName}</span>
                      </label>
                    ))
                  )}
                </div>
          ...
          {selectedAM && (
            <div className="mt-4 flex justify-end">
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 disabled:bg-gray-400 font-semibold transition-colors"
              >
                  ...
                  <tr key={accountManager.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      {assignedTLs.length > 0 ? (
                        <div className="flex flex-wrap gap-1 justify-center">
                          {assignedTLs.map(tl => (
                            <span key={tl.id} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-cyan-100 text-cyan-800">
                              {tl.fullName}
                            </span>
                          ))}
                        </div>
                    ...
                    <td className="px-4 py-3 text-center">
                      {assignedQCs.length > 0 ? (
                        <div className="flex flex-wrap gap-1 justify-center">
                          {assignedQCs.map(qc => (
                            <span key={qc.id} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800">
                              {qc.fullName}
                            </span>
                          ))}
                        </div>
