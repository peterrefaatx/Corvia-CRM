...
const TeamMemberPipeline: React.FC = () => {
  ...
  // Check permissions
  const hasFullAccess = user?.permissions?.pipeline?.full_access === true;
  const canMarkClosed = user?.permissions?.pipeline?.mark_closed === true;
  const canMarkDead = user?.permissions?.pipeline?.mark_dead === true;
  ...
  useEffect(() => {
    const init = async () => {
      await refreshUser(); // Refresh permissions on page load
      loadPipelineStages();
      loadLeads();
    };
    init();
  ...
  const loadLeads = async () => {
    try {
      const response = await api.get('/team-member/leads');
  ...
  const openScheduleModal = (lead: Lead, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedLead(lead);
  ...
  const openDetailsModal = async (leadId: string) => {
    ...
    navigate(`/team-member/leads/${leadId}`);
  };

  const openDetailsModalOld = async (leadId: string) => {
    try {
  ...
  const addNote = async () => {
    ...
    try {
      const response = await api.post(`/team-member/leads/${selectedLead.id}/notes`, {
  ...
  const updateNote = async (noteId: string) => {
    ...
    try {
      const response = await api.put(`/team-member/notes/${noteId}`, {
  ...
  const deleteNote = async (noteId: string) => {
    ...
    try {
      await api.delete(`/team-member/notes/${noteId}`);
      const updatedLead = {
        ...selectedLead,
  ...
  const markLeadAs = (stage: 'Closed' | 'Dead') => {
    if (!hasFullAccess) {
      showToast('You do not have permission to mark leads', 'error');
      return;
    }
  ...
  const confirmMarkLead = async () => {
    ...
    try {
      await api.patch(`/team-member/leads/${selectedLead.id}/archive`, { pipelineStage: confirmAction });
  ...
  const handleDrop = async (stage: PipelineStage) => {
    ...
    try {
      await api.patch(`/team-member/leads/${draggedLead.id}/stage`, {
        pipelineStage: stage
      });
  ...
  const moveToNextStage = async (lead: Lead, e: React.MouseEvent) => {
    ...
    try {
      await api.patch(`/team-member/leads/${lead.id}/stage`, {
        pipelineStage: nextStage.name
      });
  ...
  const handleStageChange = async (leadId: string, newStage: PipelineStage) => {
    try {
      await api.patch(`/team-member/leads/${leadId}/stage`, {
  ...
  const loadSchedules = async (leadId: string) => {
    try {
      const response = await api.get(`/team-member/leads/${leadId}/schedules`);
  ...
  const createSchedule = async () => {
    ...
    try {
      ...
      await api.post(`/team-member/leads/${selectedLead.id}/schedules`, {
  ...
  const updateScheduleStatus = async (scheduleId: string, status: ClientSchedule['status']) => {
    try {
      await api.patch(`/team-member/schedules/${scheduleId}`, { status });
  ...
  const deleteSchedule = async (scheduleId: string) => {
    try {
      await api.delete(`/team-member/schedules/${scheduleId}`);
  ...
  const confirmReschedule = async (scheduleId: string) => {
    ...
    try {
      ...
      await api.patch(`/team-member/schedules/${scheduleId}`, {
        ...
        <div className="bg-white border-b border-slate-100 px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              {!hasFullAccess && (
                <span className="bg-yellow-100 text-yellow-800 border border-yellow-200 px-3 py-1 rounded-full text-sm font-semibold">
                  🔒 Read-Only Access
                </span>
              )}
            </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-3">
                      {stageLeads.map((lead) => (
                        <div
                          ...
                          className={`${getQualityIndicatorBg(lead)} rounded-lg border p-4 hover:border-indigo-200 hover:shadow-sm transition-all ${hasFullAccess ? 'cursor-move' : 'cursor-pointer'} relative`}
                        >
                          {/* Stage Completion Indicator */}
                          {lead.stageCompletion?.isComplete && (
                            <div className="absolute top-2 right-2 bg-emerald-500 text-white rounded-full p-1.5 shadow-lg z-10" title="All tasks complete for this stage">
                          ...
                          <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-200">
                            ...
                            {lead.clientNotes && lead.clientNotes.length > 0 && (
                              <span className="text-xs text-primary" style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 500 }}>
                          ...
                          <div className="space-y-2">
                            <button
                              ...
                              className="w-full px-3 py-2 bg-primary text-white text-xs rounded-lg bg-primary-hover transition-colors font-medium"
                            >
                              View Details
                            </button>
                            
                            {/* Move to Next Stage Button - Only show when stage is complete and user has full access */}
                            {lead.stageCompletion?.isComplete && hasFullAccess && (
                              <button
                                onClick={(e) => moveToNextStage(lead, e)}
                                className="w-full px-3 py-2 bg-emerald-600 text-white text-xs rounded-lg hover:bg-emerald-700 transition-colors flex items-center justify-center gap-1 font-medium"
                            ...
                            <div>
                              ...
                              <div className="mt-3 flex items-center justify-between">
                                <div className="flex flex-col gap-0.5">
                                  ...
                                  {note.authorName && (
                                    <span className="text-xs text-slate-500" style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 500 }}>
                                      by {note.authorName} {note.authorType === 'team_member' ? '(Team)' : '(Client)'}
                                    </span>
                                  )}
                                </div>
                                <div className="flex gap-3">
...
export default TeamMemberPipeline;
