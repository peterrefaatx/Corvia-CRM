...
const AccountManagerAnalytics: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      if (activeTab === 'performance') {
        // Load agent performance from ACM endpoint
        ...
        // Load attendance from ACM endpoint
        const params = { 
          from: startDate, 
          to: endDate
        };
        ...
        // Load teams comparison - use ACM teams endpoint
        const teamsResponse = await api.get('/account-manager/teams');
        const acmTeams = teamsResponse.data.teams || [];
        
        // Transform ACM teams data to match analytics format
        const teamsData = acmTeams.map((team: any) => ({
          teamId: team.teamId,
          teamName: team.teamName,
          agentCount: team.totalAgents,
          total: team.monthLeads,
          qualified: team.qualified,
          qualificationRate: team.monthLeads > 0 ? ((team.qualified / team.monthLeads) * 100).toFixed(1) : '0',
          avgPerAgent: team.totalAgents > 0 ? (team.monthLeads / team.totalAgents).toFixed(1) : '0'
        }));
        
        setTeamsComparison(teamsData);
        setTeams(acmTeams.map((t: any) => ({ id: t.teamId, name: t.teamName })));
        
        // Load agents from managed teams
        const agentsData: any[] = [];
        for (const team of acmTeams) {
          try {
            const teamDetailsResponse = await api.get(`/account-manager/team/${team.teamId}`);
            if (teamDetailsResponse.data.team && teamDetailsResponse.data.team.agents) {
              teamDetailsResponse.data.team.agents.forEach((agent: any) => {
                agentsData.push({
                  id: agent.agentId,
            ...
            console.error(`Failed to load agents for team ${team.teamId}:`, error);
          }
        }
  ...
  const exportToCSV = () => {
    try {
      let csvContent = '';
      let fileName = `acm_analytics_${activeTab}_${new Date().toISOString().split('T')[0]}.csv`;
      
      if (activeTab === 'performance' && agentReports.length > 0) {
        ...
        fileName = `acm_agent_performance_${new Date().toISOString().split('T')[0]}.csv`;
      } else if (activeTab === 'attendance' && attendanceReports.length > 0) {
        // Export attendance
        csvContent = 'Name,Role,Team,Days Present,Days Late,Total Lateness (min),Avg Login Time\n';
        ...
        fileName = `acm_attendance_${new Date().toISOString().split('T')[0]}.csv`;
        ...
        fileName = `acm_teams_comparison_${new Date().toISOString().split('T')[0]}.csv`;
        ...
        fileName = `acm_team_${selectedTeamDetails.teamName}_${new Date().toISOString().split('T')[0]}.csv`;
        ...
        fileName = `acm_agent_${selectedAgent.name}_${new Date().toISOString().split('T')[0]}.csv`;
      } else {
  ...
  const exportToExcel = () => {
    try {
      ...
      let fileName = `acm_analytics_${activeTab}_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      if (activeTab === 'performance' && agentReports.length > 0) {
        ...
        fileName = `acm_agent_performance_${new Date().toISOString().split('T')[0]}.xlsx`;
      } else if (activeTab === 'attendance' && attendanceReports.length > 0) {
        ...
        fileName = `acm_attendance_${new Date().toISOString().split('T')[0]}.xlsx`;
      } else if (activeTab === 'overview' && teamsComparison.length > 0) {
        data = teamsComparison.map(team => ({
          'Team Name': team.teamName,
          'Agents': team.agentCount,
          'Total Leads': team.total,
          'Qualified': team.qualified,
          'Quality Rate': `${team.qualificationRate}%`,
          'Avg per Agent': team.avgPerAgent
        }));
        sheetName = 'Teams Comparison';
        fileName = `acm_teams_comparison_${new Date().toISOString().split('T')[0]}.xlsx`;
        ...
        fileName = `acm_agent_${selectedAgent.name}_${new Date().toISOString().split('T')[0]}.xlsx`;
      }
          ...
          <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Account manager performance insights</p>
              </div>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatCard 
                title="Total Leads" 
                value={teamsComparison.reduce((sum, t) => sum + parseInt(t.total || 0), 0)} 
                color="border-blue-500" 
              />
              <StatCard 
                title="Avg Quality" 
                value={`${(teamsComparison.reduce((sum, t) => sum + parseFloat(t.qualificationRate || 0), 0) / teamsComparison.length || 0).toFixed(1)}%`}
                color="border-blue-500" 
              />
            </div>
                  ...
                  <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-gray-300">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="bg-primary-light p-4 rounded-lg">
                        ...
                        <p className="text-3xl font-bold text-primary">{selectedTeamDetails.summary.qualificationRate}%</p>
                          ...
                          <tbody className="divide-y divide-gray-200">
                            {campaignProgress.campaigns.map((campaign: any) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-4 whitespace-nowrap text-center">
                                  <span className="campaign-badge px-3 py-1.5 bg-primary-light text-blue-800 text-xs font-semibold inline-block rounded-full">
                                    {campaign.duplicate}
                                  </span>
                                </td>
                                <td className="px-4 py-4 whitespace-nowrap text-center">
...
export default AccountManagerAnalytics;
