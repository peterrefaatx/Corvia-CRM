                ...
                {tabs.map(tab => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-lg transition-all flex items-center space-x-3 ${
                      activeTab === tab.id
                        ? 'bg-primary-light text-blue-700 border-l-4 border-blue-600'
                        : 'text-slate-700 hover:bg-slate-50 border-l-4 border-transparent'
                    }`}
                  >
                    <svg className={`w-5 h-5 flex-shrink-0 ${activeTab === tab.id ? 'text-primary' : 'text-slate-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      {getIcon(tab.icon)}
                    </svg>
            ...
            <div className="bg-white rounded-xl border border-slate-100 p-8">
              ...
              <div className="mt-8 pt-6 border-t border-slate-200 flex justify-end gap-3">
                ...
                <button
                  ...
                  disabled={saving}
                  className="px-6 py-2.5 bg-primary text-white rounded-lg bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-colors flex items-center space-x-2"
                >
                  {saving ? (
                    <>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Preview Panel */}
        <div className="space-y-6">
          <div className="border border-blue-200 rounded-xl overflow-hidden bg-gradient-to-br from-blue-50 to-blue-50">
            ...
            <div className="p-6">
              <div className="bg-white rounded-lg p-6 border-2 border-cyan-300 mb-4">
                <p className="text-sm font-semibold text-slate-600 mb-3">Current Format:</p>
                <p className="text-3xl font-mono font-bold text-blue-700 break-all">{generatePreview()}</p>
              </div>
...
const DataRetentionSettings: React.FC<{ settings: any; onChange: (key: string, value: any) => void }> = ({ settings, onChange }) => (
  <div>
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-primary-light border border-primary200 rounded-lg">
        <p className="text-sm text-primary font-medium">Note: 0 = Keep forever</p>
      </div>
      <label className="flex items-center p-3 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
        <input
          type="checkbox"
...
const BackupSettings: React.FC<{ settings: any; onChange: (key: string, value: any) => void }> = ({ settings, onChange }) => {
  ...
  const [showRestoreConfirm, setShowRestoreConfirm] = React.useState<any>(null);
  const [restoreProgress, setRestoreProgress] = React.useState<any>(null);
  ...
  const restoreBackup = async (filename: string) => {
    try {
      ...
      const pollInterval = setInterval(async () => {
        try {
          const statusResponse = await api.get(`/backup/restore-status/${jobId}`);
          const progressData = statusResponse.data;
          ...
          if (progressData.status === 'completed') {
            ...
            clearInterval(pollInterval);
            setMessage(`Restore failed: ${progressData.message}`);
            setRestoring(null);
            setTimeout(() => {
          ...
          // Only clear interval and show error if it's not a 404 (job not found yet)
          if (error.response?.status !== 404) {
            clearInterval(pollInterval);
            console.error('Failed to check restore status:', error);
            setMessage(error.response?.data?.error || 'Failed to check restore status');
            setRestoring(null);
            setRestoreProgress(null);
          }
          // If 404, just keep polling - job might not be created yet
        }
      ...
      setTimeout(() => {
        clearInterval(pollInterval);
        if (restoring) {
          setMessage('Restore is taking longer than expected. Please check manually.');
          setRestoring(null);
        }
      }, 600000);
      ...
      setMessage(error.response?.data?.error || 'Failed to start restore');
      setRestoring(null);
      setRestoreProgress(null);
    }
  };
  ...
  const getBackupTypeColor = (type: string) => {
    switch (type) {
      case 'daily': return 'bg-blue-100 text-blue-800 border border-blue-200';
      case 'monthly': return 'bg-purple-100 text-purple-800 border border-purple-200';
      case 'yearly': return 'bg-green-100 text-green-800 border border-green-200';
      case 'manual': return 'bg-amber-100 text-amber-800 border border-amber-200';
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-slate-900 mb-6">Backup & Restore</h2>
      ...
      {restoreProgress && (
        <div className="mb-6 p-6 bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-indigo-200 rounded-xl shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-indigo-900">🔄 Restoring Backup</h3>
            <span className="text-2xl font-bold text-indigo-600">{restoreProgress.progress}%</span>
          </div>
          ...
          <div className="mb-4 p-3 bg-white rounded-lg border border-indigo-200">
            <p className="text-sm font-semibold text-indigo-900 mb-1">Current Step:</p>
            <p className="text-indigo-700">{restoreProgress.currentStep}</p>
          </div>

          {/* Steps List */}
          <div className="space-y-2">
            {restoreProgress.steps?.map((step: any, index: number) => (
              <div 
                key={index}
                className={`flex items-center space-x-3 p-3 rounded-lg transition-all ${
                  step.status === 'completed' ? 'bg-green-100 border border-green-300' :
                  step.status === 'running' ? 'bg-primary-light border border-primary300 animate-pulse' :
                  step.status === 'failed' ? 'bg-red-100 border border-red-300' :
              ...
              >
                ...
                <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                  ...
                  step.status === 'running' ? 'bg-primary-light text-primary' :
          ...
          <div className="mt-4 p-3 bg-white rounded-lg border border-indigo-200">
            <div className="flex items-center justify-between text-sm">
              <span className="text-indigo-700 font-medium">
                Progress: {restoreProgress.completedSteps} of {restoreProgress.totalSteps} steps completed
              </span>
              {restoreProgress.safetyBackup && (
      ...
      <div className="mb-8 p-6 bg-white border border-slate-200 rounded-xl">
        <h3 className="text-lg font-bold text-slate-900 mb-4">Backup Configuration</h3>
        <div className="space-y-4">
          <label className="flex items-center p-3 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
            <input
              type="checkbox"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">Keep Daily Backups (days)</label>
              <input
                type="number"
          ...
          {settings.lastBackup && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm font-semibold text-green-900">Last Backup: {new Date(settings.lastBackup).toLocaleString()}</p>
            </div>
          )}
          <button
            onClick={createBackup}
            disabled={creating}
            className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 font-medium transition-colors flex items-center space-x-2"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
            </svg>
            <span>{creating ? 'Creating Complete Backup...' : 'Backup Everything Now'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="p-6 bg-white border border-slate-200 rounded-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-slate-900">Available Backups</h3>
          <button
            onClick={loadBackups}
            disabled={loading}
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-slate-500">No backups available. Create your first backup above.</div>
        ) : (
          <div className="space-y-3">
            {backups.map((backup) => (
              <div key={backup.filename} className="p-4 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <span className={`px-2 py-1 text-xs font-semibold rounded ${getBackupTypeColor(backup.type)}`}>
                        {backup.type.toUpperCase()}
                      </span>
                      <span className="text-sm font-mono text-slate-600">{backup.filename}</span>
                    </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-primary text-white rounded-lg bg-primary-hover text-sm font-medium transition-colors"
                    >
                      Download
                    </button>
                    <button
      ...
      {showRestoreConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-slate-900 mb-4">⚠️ Restore Backup?</h3>
            <p className="text-slate-600 mb-4">
              You are about to restore: <span className="font-mono font-semibold">{showRestoreConfirm.filename}</span>
            </p>
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
              <p className="text-sm text-amber-900 font-semibold mb-2">This will:</p>
              <ul className="text-sm text-amber-800 space-y-1 list-disc list-inside">
                <li>Create a safety backup of current data</li>
                <li>Replace database with backup data</li>
                <li>Take about 30-60 seconds</li>
                <li>Require page refresh</li>
              </ul>
            </div>
            <div className="flex space-x-3">
              ...
              <button
                onClick={() => restoreBackup(showRestoreConfirm.filename)}
